...
public class EmbeddedSchematronValidator {
    ...
    public void validateXmlDocument(Document d, ErrorAggregator errors) {
        try {
            ...
            for (int i = 0; i < nl.getLength() ; i ++) {
                errors.error((String) xpath.evaluate("text()", nl.item(i), XPathConstants.STRING));
            }
        } catch (TransformerException e) {
            errors.error(e.getLocalizedMessage() == null ? "Error performing schematron validation." : e.getLocalizedMessage());
        } catch (XPathExpressionException e) {
            e.printStackTrace();
            errors.error("Error parsing schematron validation result.");
        }
